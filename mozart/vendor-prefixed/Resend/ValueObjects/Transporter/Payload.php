...
namespace MozartTest\Resend\ValueObjects\Transporter;

use MozartTest\GuzzleHttp\Psr7\Request;
use InvalidArgumentException;
use MozartTest\Psr\Http\Message\RequestInterface;
use MozartTest\Resend\Resend;
use MozartTest\Resend\Enums\Transporter\ContentType;
use MozartTest\Resend\Enums\Transporter\Method;
use MozartTest\Resend\ValueObjects\ResourceUri;
final class Payload
{
    /**
     * Create a new Transporter Payload instance.
     */
    public function __construct(private readonly ContentType $contentType, private readonly Method $method, private readonly ResourceUri $uri, private readonly array $parameters = [], private ?Headers $headers = null)
    {
        //
    }
    /**
    ...
    public function toRequest(BaseUri $baseUri, Headers $headers): RequestInterface
    {
        ...
        $mergedHeaders = $mergedHeaders->withUserAgent('resend-php', Resend::VERSION)->withContentType($this->contentType);
        if ($this->method === Method::POST || $this->method === Method::PATCH || $this->method === Method::PUT) {
            $body = json_encode($this->parameters === [] || !array_is_list($this->parameters) ? (object) $this->parameters : $this->parameters, JSON_THROW_ON_ERROR);
        }
