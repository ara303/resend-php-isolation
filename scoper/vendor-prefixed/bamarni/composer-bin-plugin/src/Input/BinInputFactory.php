...
namespace ScoperTest\Bamarni\Composer\Bin\Input;

use ScoperTest\Symfony\Component\Console\Input\InputInterface;
use ScoperTest\Symfony\Component\Console\Input\StringInput;
...
final class BinInputFactory
{
    ...
    {
        $matchResult = preg_match('/^([^\s]+)(.*?\s?)(--(?: .+)?)?$/', $previousInput->__toString(), $matches);
        if (1 !== $matchResult) {
            throw InvalidBinInput::forNamespaceInput($previousInput);
        }
        $inputParts = array_filter(array_map('trim', ['--working-dir=.', $matches[1], $matches[2], $matches[3] ?? '']));
