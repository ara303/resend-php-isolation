...
namespace ScoperTest\GuzzleHttp\Handler;

use ScoperTest\GuzzleHttp\Exception\ConnectException;
use ScoperTest\GuzzleHttp\Exception\RequestException;
use ScoperTest\GuzzleHttp\Promise as P;
use ScoperTest\GuzzleHttp\Promise\FulfilledPromise;
use ScoperTest\GuzzleHttp\Promise\PromiseInterface;
use ScoperTest\GuzzleHttp\Psr7\LazyOpenStream;
use ScoperTest\GuzzleHttp\TransferStats;
use ScoperTest\GuzzleHttp\Utils;
use ScoperTest\Psr\Http\Message\RequestInterface;
use ScoperTest\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \ScoperTest\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \ScoperTest\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \ScoperTest\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \ScoperTest\GuzzleHttp\Psr7\Utils::streamFor($sink);
