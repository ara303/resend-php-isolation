...
namespace ScoperTest\Resend\Service\Contacts;

use ScoperTest\Resend\Service\Service;
use ScoperTest\Resend\ValueObjects\Transporter\Payload;
class Segment extends Service
{
    public function add(string $contact, string $segmentId): \ScoperTest\Resend\Segment
    {
     ...
     */
    public function list(string $contactId, array $options = []): \ScoperTest\Resend\Collection
    {
        $payload = Payload::list("contacts/{$contactId}/segments", $options);
        $result = $this->transporter->request($payload);
        return $this->createResource('segments', $result);
    }
    public function remove(string $contact, string $segmentId): \ScoperTest\Resend\Segment
    {
